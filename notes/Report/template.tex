...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4, angle=0, origin=c]{img/biCriteriaExcelOutput.png}
	\caption{Output of all relevant algorithms in terms of number of solution and time spent}
...
%It has to be taken only like a conjecture made for pure self-interest.}
\vspace{10mm}
...
%Removing by the set of solution those dominated and, using a \textbf{fitness function}, associate to all the remaining solution one or more parameter, based on distance, danger and travel time.
%It is not granted that from two good solutions a third one is also good and neither that from two solutions with a certain fitness values a it is generated a third one with the same fitness value. To solve these problems generally is used a criteria to chose a set from all the solution and use that for the crossover phase (for example )
%crossover nuova popolazione
